...
@Component({
  selector: 'app-adminviewdogcare',
  templateUrl: './adminviewdogcare.component.html',
  styleUrls: ['./adminviewdogcare.component.css']
})
export class AdminviewdogcareComponent implements OnInit {
  displayedColumns: string[] =
  ['Slno', 'CenterName', 'MobileNumber', 'EmailId','OwnerName','Address','Actions'];
  errorMessage: any;
  length = 100;
  pageSize = 10;
      ...
      data => {
        if (data.responseCode == "OK") {
          ...
          for (var i = 0; i < this.customerdata.length; i++) {
            displayData.push({
              "CenterName": this.customerdata[i].centerName,
              "MobileNumber": this.customerdata[i].phoneNum,
              "EmailId":this.customerdata[i].emailId,
              "OwnerName":this.customerdata[i].ownerName,
